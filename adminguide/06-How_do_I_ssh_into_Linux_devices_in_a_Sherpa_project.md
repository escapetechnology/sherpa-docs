...
![Public SSH key]({{config.baseurl}}/images/public-ssh-key.png)

When you create a Sherpa project you are encouraged to use an SSH key.

![Add an SSH key]({{config.baseurl}}/images/add-an-ssh-key.png)

Click on **Apply an existing key** and select the existing key from the dropdown.

![Apply an existing key]({{config.baseurl}}/images/apply-an-existing-key.png)
...
![Add a new key]({{config.baseurl}}/images/add-a-new-key.png)
