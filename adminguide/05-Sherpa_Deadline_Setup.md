...
![worker-settings]({{config.baseurl}}/images/worker-settings.png)
...
![pulse-settings]({{config.baseurl}}/images/pulse-settings.png)
...
![pulse-power-management]({{config.baseurl}}/images/pulse-power-management.png)

### Options

Under _Tools -> Options_ ...

![options]({{config.baseurl}}/images/options.png)

Make sure _Enable Worker Pinging_ box is ticked.

![enable-worker-ping]({{config.baseurl}}/images/enable-worker-ping.png)
...
![configure-events]({{config.baseurl}}/images/configure-events.png)

### Disable AWSPortal

Disable AWS Portal by setting the _State_ to be _Disabled_.

![disable-aws-portal]({{config.baseurl}}/images/disable-aws-portal.png)
...
![configure-sherpa-event-plugin]({{config.baseurl}}/images/configure-sherpa-event-plugin.png)

### Configure Power Management

Select _Tools -> Configure Power Management ..._

![configure-power-management]({{config.baseurl}}/images/configure-power-management.png)
...
![add-power-management-group]({{config.baseurl}}/images/add-power-management-group.png)
...
![configure-idle-shutdown]({{config.baseurl}}/images/configure-idle-shutdown.png)
...
![configure-machine-startup]({{config.baseurl}}/images/configure-machine-startup.png)
...
![idle-shutdown-01]({{config.baseurl}}/images/idle-shutdown-01.png)

Zooming in on some of the important detail from the screen-grab above ...

A. The render node is on-line and pingable.
![idle-shutdown-01a]({{config.baseurl}}/images/idle-shutdown-01a.png)
...
![idle-shutdown-01b]({{config.baseurl}}/images/idle-shutdown-01b.png)
...
![idle-shutdown-01c]({{config.baseurl}}/images/idle-shutdown-01c.png)

In the example below I have set the Idle Shutdown time to be 2 minutes.
![idle-shutdown-02]({{config.baseurl}}/images/idle-shutdown-02.png)

Hence after 2.5 minutes the render node has been switched off and is now being reported as _Timed Out_.
![idle-shutdown-02a]({{config.baseurl}}/images/idle-shutdown-02a.png)
...
![idle-shutdown-03]({{config.baseurl}}/images/idle-shutdown-03.png)

So 5 minutes later the render node turns red and gets marked as _Stalled_ (or _Offline_).
![idle-shutdown-03a]({{config.baseurl}}/images/idle-shutdown-03a.png)
